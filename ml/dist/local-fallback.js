...
exports.createLocalPlan = createLocalPlan;
exports.createLocalDesign = createLocalDesign;
exports.createLocalContent = createLocalContent;
exports.createLocalCode = createLocalCode;
...
function createLocalPlan(prompt) {
    ...
    if (lower.includes("about"))
        sections.push("about");
    if (lower.includes("skills") || isPortfolio)
        sections.push("skills");
    if (lower.includes("projects") || isPortfolio)
        sections.push("projects");
    if (lower.includes("menu") || isRestaurant)
        sections.push("menu");
    if (lower.includes("pricing") || isSaas)
        sections.push("pricing");
    if (lower.includes("testimonials") || lower.includes("reviews"))
...
function createLocalDesign(plan) {
    return {
...
function createLocalContent(plan, prompt) {
    const lower = prompt.toLowerCase();
...
function createLocalCode(plan, design, content) {
    const html = (0, generator_js_1.renderHtml)(plan, design, content);
    return {
        html,
        files: (0, generator_js_1.createFiles)(plan, design, content, html),
        assets: [
